...
unsigned char empty_buf[20] = {0x1E,0x9D,0x66,0x00,0x00,0x39,0xCE,0x70,0x00,0x1C,0xE7,0x38,0x00,0x0E,0x73,0x9C,0x00,0x07,0x39,0xCE};
...
unsigned char get_can_frame(unsigned char *ptr) {
	...
	if((can_wr_pos<FRAME_COUNT) && can_frames[0].ready) {
		for(i=0;i<20;i++) ptr[i] = can_frames[0].buf[i];
		for(i=0;i<FRAME_COUNT-1;i++) {
		...
		for(i=0;i<20;i++) can_frames[FRAME_COUNT-1].buf[i] = empty_buf[i];
